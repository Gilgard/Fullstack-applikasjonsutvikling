...
describe('LoginComponent.vue', () => {
  ...
  it('Writing in text fields writes', async () => {
    const wrapper = shallowMount(LoginComponent)

    const userfield = wrapper.find('#userfield')
    await userfield.setValue('user')
    expect(userfield.element.value).toBe('user')

    const passfield = wrapper.find('#passfield')
    await passfield.setValue('pass')
    expect(passfield.element.value).toBe('pass')
  })
})
